...
Table \ref{tab:corr} reports the correlations between dependent and independent variables. As expected, it features a strong positive correlation between national income and life expectancy. 
...
Figure \ref{fig:yearlyplot} indicates although life expectancy positively relates to GDP per capita in general, the growth rate is not exactly corresponding to each other. Especially from 2007 to 2009, there is a significant decrease in GDP per capita but the life expectancy still goes up at a steady speed.

```{r yearlyplot, echo = FALSE, fig.align="center",fig.cap="\\label{fig:scatter}GDP Capita and Life Expectancy"}
fig_yearlyplot
```
